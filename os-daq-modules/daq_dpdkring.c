...
static int start_instance(Dpdk_Context_t *dpdkc, DpdkInstance *instance)
{
    ...
    instance->rx_ring = rte_ring_lookup(instance->rx_name);

    if (instance->rx_ring == NULL)
    {
        DPE(dpdkc->errbuf, "%s: Cannot get RX ring (%s)\n", __FUNCTION__, instance->rx_name);
        return DAQ_ERROR;
    }
    ...
    else
    {
        ...
        instance->tx_ring_peer = rte_ring_lookup(instance->tx_name);
        instance->tx_ring_reverse = rte_ring_lookup(instance->tx_reverse_name);

        if (instance->tx_ring_peer == NULL || instance->tx_ring_reverse == NULL)
        {
            DPE(dpdkc->errbuf, "%s: (Peer mode) Cannot get both TX peer ring (%s) and TX reverse ring (%s)\n", __FUNCTION__, instance->tx_name, instance->tx_reverse_name);
            return DAQ_ERROR;
        }
    }
...
static DpdkInstance *create_instance(const char *device, const char *peer, DpdkInstance *parent, char *errbuf, size_t errlen, char* poolname)
{
...
static int dpdkring_daq_initialize(const DAQ_Config_t *config, void **ctxt_ptr, char *errbuf, size_t errlen)
{
    ...
    snprintf(poolname, sizeof(poolname), "click_mempool_snort_%d", 0);
    ...
    for (entry = config->values; entry; entry = entry->next)
    {
        ...
        if (!strcmp(entry->key, "dpdk_pool"))
            strcpy(poolname, entry->value);
    }
        ...
        else
        {
            ...
            if (dpdkc->peer_mode)
            {
                ...
                else
                {
                    ...
                    instance = create_instance(intf, intf_peer, dpdkc->instances, errbuf, errlen, poolname);
            ...
            {
                snprintf(intf, len+1, "%s", dev);
                instance = create_instance(intf, NULL, dpdkc->instances, errbuf, errlen, poolname);
                called_instance_creation = 1;
            }
            ...
            if (called_instance_creation)
            {
                if (!instance)
                    goto err;
                 snprintf(instance->rx_name, sizeof(instance->rx_name), "dpdk%d_2_snort", instance->ingress_index);
                snprintf(instance->tx_name, sizeof(instance->tx_name), "snort_2_dpdk%d", instance->egress_index);
                snprintf(instance->tx_reverse_name, sizeof(instance->tx_reverse_name), "snort_2_dpdk%d", instance->ingress_index);
...
#if RTE_VERSION >= RTE_VERSION_NUM(17,5,0,0)
                unsigned left;
                const uint16_t nb_sent = rte_ring_enqueue_burst(instance->tx_ring_peer, (void *)&instance->tx_peer_burst[instance->tx_peer_start], burst_size, &left);
#else
